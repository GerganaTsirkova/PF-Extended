Lab: Arrays, Lists, Array and List Algorithms
...
⦁	Since every element is a Match now, we can extract just the Value property of the match itself, which holds the match value as a string, using Select(). We can also Trim() the value, to get rid of any leading or trailing spaces.
...
⦁	Has either whitespace before it or the end of the string (match either $ or what’s called a ⦁	positive⦁	 look⦁	ahead). The syntax for the end of the RegEx might look something like “($|(?=\s))”.
